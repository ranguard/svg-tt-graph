...
[% IF config.show_y_labels %]
  [% FOREACH field = config.fields %]
    [% IF count == 0 %]
      <text x="[% x - 10 %]" y="[% base_line - (dh / 2) %]" class="yAxisLabels">[% config.y_label_formatter(field) %]</text>
      [% i = i - dh %]
    [% ELSE %]
      <text x="[% x - 10 %]" y="[% base_line - i - (dh / 2) %]" class="yAxisLabels">[% config.y_label_formatter(field) %]</text>
    [% END %]
    [% i = i + dh %]
    [% count = count + 1 %]
  [% END %]
